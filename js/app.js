/* ============================================================
   ...
   Inline editing with clear UI controls:
   - Visible buttons for Replace / Crop on media
   - Drag handle on every movable element
   - Click to edit text
...
(function () {
  ...
  if (isCms) {
    window.addEventListener("message", function (e) {
      ...
      if (e.data.type === "CMS_CONTENT" && e.data.content) {
        content = e.data.content; if (e.data.pageSlug) currentSlug = e.data.pageSlug;
  ...
  function clearAll() {
    ...
    document.querySelectorAll(".cms-action-bar").forEach(function (b) { b.remove(); });
  }
  ...
  function renderHero(d) {
    ...
    if (c) { c.innerHTML = ""; if (d.image) { var img = document.createElement("img"); img.className = "hero__image"; img.src = resolveUrl(d.image); img.alt = ""; img.loading = "eager"; if (d.imagePosition) img.style.objectPosition = d.imagePosition.x + "% " + d.imagePosition.y + "%"; c.appendChild(img); } }
    var badge = document.querySelector(".hero__badge"); if (badge && d.badge) badge.textContent = d.badge;
    applyPos("#hero-title", d.titlePosition);
    applyPos("#hero-subtitle", d.subtitlePosition);
    applyPos(".hero__content", d.contentPosition);
    addMediaButtons("hero-media-zone", "hero", "hero-media", "hero", "imagePosition");
    addDragHandle("#hero-title", "hero", "titlePosition");
    addDragHandle("#hero-subtitle", "hero", "subtitlePosition");
    addDragHandle(".hero__badge", "hero", "badgePosition");
  }
  ...
  function renderVideoLoop(d) {
    ...
    if (c) { c.innerHTML = ""; if (d.video) { var v = document.createElement("video"); v.src = resolveUrl(d.video); v.autoplay = true; v.muted = true; v.loop = true; v.playsInline = true; v.preload = "auto"; v.setAttribute("playsinline", ""); if (d.videoPosition) v.style.objectPosition = d.videoPosition.x + "% " + d.videoPosition.y + "%"; c.appendChild(v); v.play().catch(function () {}); } }
    applyPos("#video-loop-title", d.titlePosition);
    addMediaButtons("video-loop-media", "videoLoop-video", "video-loop-media", "videoLoop", "videoPosition");
    addDragHandle("#video-loop-title", "videoLoop", "titlePosition");
  }

  /* ── video play ──────────────────────────────────── */
  function renderVideoPlay(d) {
    show("videoPlay"); setTxt("video-play-title", d.title);
    var lbl = document.querySelector(".video-play__label"); if (lbl && d.label) lbl.textContent = d.label;
    var c = document.getElementById("video-play-media");
    if (c) { var glow = c.querySelector(".video-play__glow"); c.innerHTML = ""; if (glow) c.appendChild(glow); if (d.video) { var v = document.createElement("video"); v.src = resolveUrl(d.video); v.controls = true; v.playsInline = true; v.preload = "auto"; v.setAttribute("playsinline", ""); if (d.poster) v.poster = resolveUrl(d.poster); c.appendChild(v); } }
    addMediaButtons("video-play-media", "videoPlay-video", null, null, null, true);
    addDragHandle("#video-play-title", "videoPlay", "titlePosition");
  }

  /* ── about ──────────────────────────────────────── */
  function renderAbout(d) {
    show("about"); setTxt("about-title", d.title); setTxt("about-text", d.text);
    var ey = document.querySelector(".about__eyebrow"); if (ey && d.eyebrow) ey.textContent = d.eyebrow;
    var c = document.getElementById("about-media");
    if (c) { c.innerHTML = ""; if (d.image) { var img = document.createElement("img"); img.className = "about__image"; img.src = resolveUrl(d.image); img.alt = ""; if (d.imagePosition) img.style.objectPosition = d.imagePosition.x + "% " + d.imagePosition.y + "%"; c.appendChild(img); } }
    applyPos("#about-title", d.titlePosition);
    applyPos("#about-text", d.textPosition);
    addMediaButtons("about-media-zone", "about", "about-media", "about", "imagePosition");
    addDragHandle("#about-title", "about", "titlePosition");
    addDragHandle("#about-text", "about", "textPosition");
  }

  /* ── services ──────────────────────────────────── */
  function renderServices(d) {
    show("services"); setTxt("services-title", d.title);
    var ey = document.querySelector(".services__eyebrow"); if (ey && d.eyebrow) ey.textContent = d.eyebrow;
  ...
  function renderContact(d) {
    ...
    if (cta) { cta.textContent = d.cta || d.buttonLabel || ""; cta.href = d.email ? "mailto:" + d.email : "#"; }
    applyPos("#contact-title", d.titlePosition);
    applyPos("#contact-cta", d.ctaPosition);
    addDragHandle("#contact-title", "contact", "titlePosition");
    addDragHandle("#contact-cta", "contact", "ctaPosition");
  }
  ...
  function wireServiceCards() {
    ...
    list.querySelectorAll(".service-card").forEach(function (card, idx) {
      ...
      [{ sel: ".service-card__title", f: "title" }, { sel: ".service-card__description", f: "description" }].forEach(function (cfg) {
        ...
        function emit() {
          var d = pageData(currentSlug); if (!d || !d.services || !d.services.items || !d.services.items[idx]) return;
          var items = d.services.items.map(function (it) { return Object.assign({}, it); }); items[idx][field] = el.textContent;
  ...
  function wireEditors() {
    ...
    wireText("services-title","services","title"); wireText("contact-title","contact","title");
    wireText("contact-text","contact","text"); wireText("contact-email","contact","email");
    wireCta();
  ...
  /* ============================================================
     MEDIA ACTION BUTTONS (visible on hover)
     ============================================================ */
  function addMediaButtons(zoneId, uploadKey, cropContainerId, cropSection, cropPosField, hasPoster) {
    if (!isCms) return;
    var zone = document.getElementById(zoneId); if (!zone) return;
    if (zone.querySelector(".cms-action-bar")) return;

    var bar = document.createElement("div"); bar.className = "cms-action-bar";

    var btnReplace = document.createElement("button"); btnReplace.className = "cms-btn"; btnReplace.innerHTML = "&#x1F4F7; Remplacer";
    btnReplace.addEventListener("click", function (e) { e.stopPropagation(); e.preventDefault(); postToParent({ type: "CMS_UPLOAD_REQUEST", source: "cms-site", uploadKey: uploadKey }); });
    bar.appendChild(btnReplace);

    if (cropContainerId) {
      var btnCrop = document.createElement("button"); btnCrop.className = "cms-btn"; btnCrop.innerHTML = "&#x2702; Recadrer";
      btnCrop.addEventListener("mousedown", function (e) {
        e.stopPropagation(); e.preventDefault();
        var container = document.getElementById(cropContainerId);
        if (container) startCrop(container, cropSection, cropPosField, e.clientX, e.clientY);
      });
      bar.appendChild(btnCrop);
    }

    if (hasPoster) {
      var btnPoster = document.createElement("button"); btnPoster.className = "cms-btn"; btnPoster.innerHTML = "&#x1F5BC; Miniature";
      btnPoster.addEventListener("click", function (e) { e.stopPropagation(); e.preventDefault(); postToParent({ type: "CMS_UPLOAD_REQUEST", source: "cms-site", uploadKey: "videoPlay-poster" }); });
      bar.appendChild(btnPoster);
    }
  ...
  /* ============================================================
     DRAG HANDLE (visible move grip on each element)
     ============================================================ */
  function addDragHandle(selector, section, posField) {
    if (!isCms) return;
    var el = (typeof selector === "string") ? document.querySelector(selector) : selector;
    if (!el || el.querySelector(".cms-drag-handle")) return;
    el.style.position = el.style.position || "relative";

    var handle = document.createElement("div"); handle.className = "cms-drag-handle"; handle.title = "Déplacer"; handle.innerHTML = "&#x2630;";
    handle.addEventListener("mousedown", function (e) { e.stopPropagation(); e.preventDefault(); startMove(el, section, posField, e.clientX, e.clientY); });
    handle.addEventListener("touchstart", function (e) { if (e.touches.length !== 1) return; e.stopPropagation(); startMove(el, section, posField, e.touches[0].clientX, e.touches[0].clientY); }, { passive: true });
    el.appendChild(handle);
  }

  function addCardDragHandle(card, idx) {
    if (!isCms || card.querySelector(".cms-drag-handle")) return;
    card.style.position = card.style.position || "relative";
    var handle = document.createElement("div"); handle.className = "cms-drag-handle"; handle.title = "Déplacer la carte"; handle.innerHTML = "&#x2630;";
    handle.addEventListener("mousedown", function (e) { e.stopPropagation(); e.preventDefault(); startCardMove(card, idx, e.clientX, e.clientY); });
    handle.addEventListener("touchstart", function (e) { if (e.touches.length !== 1) return; e.stopPropagation(); startCardMove(card, idx, e.touches[0].clientX, e.touches[0].clientY); }, { passive: true });
  ...
  function startCrop(container, section, posField, cx, cy) {
    ...
    var style = window.getComputedStyle(media); var pos = style.objectPosition || "50% 50%"; var parts = pos.split(/\s+/);
    ...
    container.classList.add("cms-cropping"); document.body.style.userSelect = "none"; document.body.style.cursor = "grabbing";
  ...
  function handleCropMove(cx, cy) {
    ...
    var nx = clamp(cropState.px - dx * 0.15, 0, 100), ny = clamp(cropState.py - dy * 0.15, 0, 100);
    var s = snapVal(nx, ny);
    cropState.media.style.objectPosition = s.x + "% " + s.y + "%"; updateSnapUI(s.x, s.y);
  }
  ...
  function startMove(el, section, posField, cx, cy) {
    moveState = { el: el, section: section, posField: posField, sx: cx, sy: cy, ox: parseFloat(el.dataset.cmsPosX) || 0, oy: parseFloat(el.dataset.cmsPosY) || 0 };
  ...
  function handleMove(cx, cy) {
    var nx = moveState.ox + (cx - moveState.sx), ny = moveState.oy + (cy - moveState.sy);
    var t = "translate(" + nx + "px, " + ny + "px)";
    moveState.el.style.transform = t; moveState.el.style.setProperty("--cms-translate", t);
    moveState.el.dataset.cmsPosX = nx; moveState.el.dataset.cmsPosY = ny;
  }
  ...
  /* ── card move (stores position inside items array) ── */
  var cardMoveState = null;
  function startCardMove(card, idx, cx, cy) {
    cardMoveState = { card: card, idx: idx, sx: cx, sy: cy, ox: parseFloat(card.dataset.cmsPosX) || 0, oy: parseFloat(card.dataset.cmsPosY) || 0 };
  ...
  document.addEventListener("mousemove", function (e) {
    ...
    var nx = cardMoveState.ox + (e.clientX - cardMoveState.sx), ny = cardMoveState.oy + (e.clientY - cardMoveState.sy);
    var t = "translate(" + nx + "px, " + ny + "px)";
    cardMoveState.card.style.transform = t; cardMoveState.card.dataset.cmsPosX = nx; cardMoveState.card.dataset.cmsPosY = ny;
  });
  document.addEventListener("touchmove", function (e) {
    if (!cardMoveState || e.touches.length !== 1) return; e.preventDefault();
    var nx = cardMoveState.ox + (e.touches[0].clientX - cardMoveState.sx), ny = cardMoveState.oy + (e.touches[0].clientY - cardMoveState.sy);
    cardMoveState.card.style.transform = "translate(" + nx + "px, " + ny + "px)"; cardMoveState.card.dataset.cmsPosX = nx; cardMoveState.card.dataset.cmsPosY = ny;
  }, { passive: false });
  ...
  var SNAP_PTS = [0, 25, 50, 75, 100], SNAP_T = 4;
  ...
  if (isCms) {
    ...
    css.textContent = [
      ...
      '.cms-action-bar { position: absolute; top: 10px; left: 50%; transform: translateX(-50%); display: flex; gap: 6px; z-index: 30; opacity: 0; transition: opacity .25s; pointer-events: none; }',
      '*:hover > .cms-action-bar { opacity: 1; pointer-events: auto; }',
      '.cms-btn { display: flex; align-items: center; gap: 5px; padding: 6px 14px; font-size: 12px; font-family: var(--sans); font-weight: 500; color: #fff; background: rgba(10,10,13,.85); border: 1px solid rgba(196,165,90,.4); border-radius: 8px; cursor: pointer; white-space: nowrap; backdrop-filter: blur(8px); transition: background .15s, border-color .15s; }',
      '.cms-btn:hover { background: rgba(196,165,90,.25); border-color: var(--gold); }',

      '.cms-drag-handle { position: absolute; top: -4px; left: -24px; width: 20px; height: 20px; display: flex; align-items: center; justify-content: center; font-size: 12px; color: var(--gold); background: rgba(10,10,13,.85); border: 1px solid rgba(196,165,90,.35); border-radius: 5px; cursor: grab; opacity: 0; transition: opacity .2s; z-index: 25; user-select: none; -webkit-user-select: none; }',
      '*:hover > .cms-drag-handle { opacity: 1; }',
      '.cms-drag-handle:hover { background: rgba(196,165,90,.3); border-color: var(--gold); }',
      ...
      '@media (max-width: 680px) {',
        '.cms-drag-handle { top: -4px; left: -18px; width: 16px; height: 16px; font-size: 10px; }',
        '.cms-action-bar { top: 6px; gap: 4px; }',
        '.cms-btn { padding: 5px 10px; font-size: 11px; }',
      '}',
    ].join('\n');
